...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            // Cada vez que se marca o desmarca un checkbox, actualizamos el texto del botón
            $('#<%= cblEmpresa.ClientID %> input[type=checkbox]').on('change', function () {
                var selected = [];

                // Recorremos los checkboxes seleccionados
                $('#<%= cblEmpresa.ClientID %> input[type=checkbox]:checked').each(function () {
                    selected.push($(this).next('label').text()); // Agregar el texto al array selected
                });

                // Actualizamos el texto del botón con las empresas seleccionadas
                var buttonText = selected.length > 0 ? selected.join(', ') : 'Todas';
                $('#dropdownEmpresa').text(buttonText);
            });
        });
        $(document).ready(function () {
            // Cada vez que se marca o desmarca un checkbox, actualizamos el texto del botón
            $('#<%= cblBarrio.ClientID %> input[type=checkbox]').on('change', function () {
                ...
                // Recorremos los checkboxes seleccionados
                $('#<%= cblBarrio.ClientID %> input[type=checkbox]:checked').each(function () {
                    selected.push($(this).next('label').text()); // Agregar el texto al array selected
                });

                // Actualizamos el texto del botón con las empresas seleccionadas
                var buttonText = selected.length > 0 ? selected.join(', ') : 'Todos';
                $('#dropdownBarrio').text(buttonText);
            });
        });

        $(document).ready(function () {
            // Recuperamos las empresas seleccionadas del localStorage al cargar la página
            var selected = JSON.parse(localStorage.getItem('selectedEmpresas')) || [];

            // Al cargar la página, actualizamos el texto del botón con los valores almacenados
            updateButtonText(selected);

            // Cada vez que se marca o desmarca un checkbox, actualizamos la lista seleccionada
            $('#<%= cblEmpresa.ClientID %> input[type=checkbox]').on('change', function () {
                selected = []; // Reseteamos el array de seleccionados

                // Recorremos los checkboxes seleccionados y los agregamos al array
                $('#<%= cblEmpresa.ClientID %> input[type=checkbox]:checked').each(function () {
                    selected.push($(this).next('label').text());
                });

                // Guardamos los valores seleccionados en localStorage
                localStorage.setItem('selectedEmpresas', JSON.stringify(selected));
            ...
            function updateButtonText(selected) {
                var buttonText = selected.length > 0 ? selected.join(', ') : 'Todas';
                $('#dropdownEmpresa').text(buttonText);
            }
        });
        ...
        $(document).ready(function () {
            // Recuperamos las empresas seleccionadas del localStorage al cargar la página
            var selected = JSON.parse(localStorage.getItem('selectedBarrios')) || [];
            ...
            $('#<%= cblBarrio.ClientID %> input[type=checkbox]').on('change', function () {
                ...
                // Guardamos los valores seleccionados en localStorage
                localStorage.setItem('selectedBarrios', JSON.stringify(selected));
        ...
        function limpiarFiltros() {
            // Limpiamos los valores guardados en localStorage
            localStorage.removeItem('selectedEmpresas');
            localStorage.removeItem('selectedBarrios');

            // Desmarcamos todos los checkboxes para empresas
            $('#<%= cblEmpresa.ClientID %> input[type=checkbox]').prop('checked', false);

            // Desmarcamos todos los checkboxes para barrios
            $('#<%= cblBarrio.ClientID %> input[type=checkbox]').prop('checked', false);

            // Restablecemos los textos de los botones
            $('#dropdownEmpresa').text('Todas');
            $('#dropdownBarrio').text('Todos');
        }

        $(document).ready(function () {
            // Al cargar la página, inicializamos los valores de los botones si no hay nada seleccionado
            ...
            $('#dropdownEmpresa').text(empresasSeleccionadas.length > 0 ? empresasSeleccionadas.join(', ') : 'Todas');
            $('#dropdownBarrio').text(barriosSeleccionados.length > 0 ? barriosSeleccionados.join(', ') : 'Todos');
        });
</script>
